...
def source_term(x, y):
    return (2 * (4 * np.pi)**2 - k**2) * exact_solution(x, y)
...
for elem in elements:
    ...
    Ke = Area * (B.T @ B - k**2 / 12 * (np.ones((3, 3)) + np.eye(3)))
    centroid = np.mean(xy, axis=0)
    f_val = source_term(centroid[0], centroid[1])
...
U = rgdbek(A_sp, b_vec, max_iter=100000, tol=1e-6, eta=0.5)
...
def plot_solution_with_mesh(vals, title):
    ...
    tpc = ax.tripcolor(triang, vals, shading='gouraud', cmap='inferno')
...
fig1.savefig('helmholtz_FEM.png', dpi=600)
fig2.savefig('helmholtz_Exact.png', dpi=600)
fig3.savefig('helmholtz_Error.png', dpi=600)
